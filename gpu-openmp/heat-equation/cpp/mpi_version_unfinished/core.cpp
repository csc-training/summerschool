...
void enter_data(Field& curr, Field& prev)
{
  ...
  nx = curr.nx;
  ny = curr.ny;
  // Unstructured data regions enable one to handle cases where memory allocation and deallocation 
  // is done in a different scope. Enter data defines the start of an unstructured data region.
  #pragma omp target enter data map(to: currdata[0:(nx+2)*(ny+2)], prevdata[0:(nx+2)*(ny+2)])  // Allocate currdata and prevdata in the device and copy from host
}
...
void exit_data(Field& curr, Field& prev)
{
  ...
  #pragma omp target exit data map(from: currdata[0:(nx+2)*(ny+2)], prevdata[0:(nx+2)*(ny+2)])
}

// Update the data in the host
void update_host_data(Field& curr){
  double* currdata = curr.temperature.data();
  int nx, ny;
  nx = curr.nx;
  ny = curr.ny;
  #pragma omp target update from(currdata[0:(nx+2)*(ny+2)])
}
