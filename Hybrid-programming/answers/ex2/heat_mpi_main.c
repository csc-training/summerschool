...
int main(int argc, char **argv)
{
    ...
    double start_time;        //!< Time stamps
    int provided;            //!< MPI support level
    ...
    /* Check that the MPI implementation supports MPI_THREAD_FUNNELED */
    if (provided < MPI_THREAD_FUNNELED) {
        printf("MPI version does not support MPI_THREAD_FUNNELED\n");
        MPI_Finalize();
        return 0;
    }
    ...
    for (iter = 1; iter <= nsteps; iter++) {
        ...
        /* Output once in every image_interval iterations, not first and
         * final which are done separately. For real perf. measurement we do
         * not want to do output */
        if (iter % image_interval == 0 && iter != 0 && iter != nsteps) {
    ...
    /* Output of final state*/
    output(&current, nsteps, &parallelization);
