...
program heat_solve
  ...
  if (provided < MPI_THREAD_FUNNELED) then
     write(*,*) 'MPI version does not support MPI_THREAD_FUNNELED'
  ...
  do iter = 1, nsteps
     ...
     if (mod(iter, image_interval) == 0 .and. iter .ne. 0 .and. iter .ne. nsteps ) then
  ...
  !final state
  call output(current, nsteps, parallelization)
