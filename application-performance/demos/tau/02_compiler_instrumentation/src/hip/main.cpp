...
int main(int argc, char **argv)
{
    ...
    bool has_support_for_gpu_aware_mpi = false;
#if defined(OMPI_HAVE_MPI_EXT_CUDA) && OMPI_HAVE_MPI_EXT_CUDA
    has_support_for_gpu_aware_mpi = (bool)MPIX_Query_cuda_support();
#else
    if (const char *env_gpu_mpi = std::getenv("MPICH_GPU_SUPPORT_ENABLED")) {
        has_support_for_gpu_aware_mpi = (bool)std::atoi(env_gpu_mpi);
    }
#endif

    if (!has_support_for_gpu_aware_mpi) {
        printf("GPU aware MPI required\n");
        fflush(stdout);
