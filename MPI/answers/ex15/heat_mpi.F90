...
contains
  ...
  subroutine write_restart(temp, parallel, iter)
    ...
    call mpi_file_write_at_all(fp, disp, temp%data, (temp%nx + 2) * (temp%ny + 2), & 
  ...
  subroutine read_restart(temp, temp0, parallel, step0)
    ...
    if (parallel%rank == 0) then
       ...
       disp = disp + sizeof(full_ny)
       call mpi_file_read_at(fp, disp, step0, 1, MPI_INTEGER, MPI_STATUS_IGNORE, ierr)
    end if
    call mpi_bcast(full_nx, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call mpi_bcast(full_ny, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    call mpi_bcast(step0, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    ...
    disp = disp + parallel%rank * (temp0%nx + 2) * (temp0%ny + 2) * sizeof(temp0%data(1,1))
    call mpi_file_read_at_all(fp, disp, temp0%data, (temp0%nx + 2) * (temp0%ny + 2), &
                         MPI_DOUBLE_PRECISION, MPI_STATUS_IGNORE, ierr)
    call mpi_file_close(fp, ierr)
