...
module heat_mpi
  ...
  type :: field
     ...
     ! TODO: Add here a field for the rma window. Example answer uses
     !       a variable with name "rma_window"
  end type field
...
contains
  ...
  subroutine initialize(field0, parallel)
    ...
    ! TODO: create the RMA window using mpi_win_create. Expose the whole
    !       array including the ghost layers.
  ...
  subroutine swap_fields(curr, prev)
    ...
    ! TODO: The RMA windows are now pointing to wrong locations.
  ...
  subroutine exchange(field0, parallel)
    ...
    ! TODO: Implement boundary exchange using one-sided communication
