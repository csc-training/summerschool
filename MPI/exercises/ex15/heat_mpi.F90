...
contains
  ...
  subroutine initialize(field0, parallel)
    ...
    do j = 0, field0%ny + 1
       do i = 0, field0%nx + 1
          ds2 = int((i - field0%nx_full / 2.0 + 1)**2 + &
               & (j + parallel%rank * field0%ny - field0%ny_full / 2.0 + 1)**2)
          if (ds2 < radius2) then
             field0%data(i,j) = 5.0
          else
  ...
  subroutine write_restart(temp, parallel, iter)
    ! Complete the TODO's in this function for part a)
    implicit none
    ...
    ! TODO Open the file called "HEAT_RESTART.dat"

    ! TODO rank 0 writes out a header comprising three values: 
    ! temp%nx_full
    ! temp%ny_full
    ! iter

    ! TODO All processes writes out their temperature data into the file, this
    ! is the temp%data array including the ghost layers. Use a
    ! collective write.
  ...
  subroutine read_restart(temp, temp0, parallel, step0)
    ! Complete the TODO's in this function for part b)
    implicit none
    ...
    ! TODO rank 0 reads the header comprising the three values (full_nx,
    ! full_ny, iter), and broadcasts the number to the other
    ! ranks. Alterntively you can also use collective reads froma ll ranks.
    ...
    ! TODO All processes reads their temperature data from the file into the
    ! temp0%data array. Use a collective write.

    ! TODO close file
    call copy_fields(temp0, temp)
