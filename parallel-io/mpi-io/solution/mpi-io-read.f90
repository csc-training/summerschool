program pario
  ...
  call mpiio_reader()
  write(*,*) 'Rank', my_id, 'read: first element', localvector(1), &
         &     'last element', localvector(localsize)
...
contains
  ...
  subroutine mpiio_reader()
    ...
    call mpi_file_open(MPI_COMM_WORLD, 'mpiio.dat', &
         & MPI_MODE_RDONLY, MPI_INFO_NULL, fh, rc)
    call mpi_file_read_at_all(fh, offset, localvector, localsize, &
