...
def plot_runtimes(filename):
    ...
    ax2.set_title("CPU multithread style strided loop", fontsize=40)
    ax3.set_title("Vectorized loads, strided loop", fontsize=40)
    ax4.set_title("Base kernel", fontsize=40)
    ...
    ax4.set_ylabel(r"absolute runtime [$\mu$s]", fontsize=30)

    for i in np.arange(0, num_taylor_iters):
        start = int(i * num_vec_sizes)
        stop = int((i + 1) * num_vec_sizes)

        ax1.semilogx(vec_size[start:stop], relative_gpu[start:stop], 'o-', lw=3.0, label='Taylor N = ' + str(1 << int(i)))
        ax2.semilogx(vec_size[start:stop], relative_cpu[start:stop], 'o-', lw=3.0)
        ax3.semilogx(vec_size[start:stop], relative_vec[start:stop], 'o-', lw=3.0)
        ax4.semilogx(vec_size[start:stop], microseconds[start:stop], 'o-', lw=3.0)
