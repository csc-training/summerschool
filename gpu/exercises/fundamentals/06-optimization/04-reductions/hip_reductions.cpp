...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-N" && i+1 < argc) {
            ...
            std::cout <<
                "Usage: --mode [naive|blockatomic|shared|optimized|unrolled|vec4|nontemporal|multiwave|hipcub]\n"
                "       --N <elements>\n"
                "       --threads <blockDim>\n"
    ...
    if (blocks <= 0) {
        ...
        blocks = std::max( (int)(20 * mp), 1 );
        // But don't exceed N coverage for 2x loading kernels
        int max_blocks = (int)((N + (size_t)threads*2 - 1) / ((size_t)threads*2));
        if (mode == M_UNROLLED) {
            max_blocks = (int)((N + (size_t)threads*8 - 1) / ((size_t)threads*8));
        } else if (mode == M_VEC4) {
            max_blocks = (int)(( (N/4) + threads - 1 ) / threads);
        }
        if (mode != M_MULTIWAVE)
            blocks = std::min(blocks, std::max(max_blocks, 1));
    ...
    for (int r = 0; r < repeat; ++r) {
        ...
        last_result = run_kernel(d_in, N, mode,
                                 mode==M_MULTIWAVE ? 1 : blocks, threads);
