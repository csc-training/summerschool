...
1. See `axpy-malloc.cpp`.
   ...
   As the axpy operation is memory bound, the execution time is limited by the memory bandwidth, and 2 threads seems to be able to saturate the 2 memory channels in a NUMA domain.

   Let's correlate the runtimes with the memory bandwidth. A single array is 0.763 MiB (`102400000*8/1024**3`). There is 2 reads and 1 write in a single axpy operation, so the total memory traffic is 38 GiB/s (`0.763*3/60e-3`).
   This is close to the maximum theoretical bandwidth of 48 GiB/s for two memory channels of the corresponding [AMD EPYC 7H12](https://en.wikichip.org/wiki/amd/epyc/7h12) processor.
