...
- OpenMP is mainly programmed through compiler directives, i.e., language extensions
  - Also runtime library routines exist --- covered later
- OpenMP directives start with `#pragma omp` / `!$omp`, followed by the directive name (e.g., `parallel`) and optional clauses (none for now --- covered later)
- Directives are ignored when the code is compiled without OpenMP support
...
- An OpenMP program can be executed as a normal executable
  - The runtime decides the number of threads and their physical location in the processor
- The `OMP_NUM_THREADS` environment variable can be used to change the number of threads
  - For example, set `export OMP_NUM_THREADS=4` before executing the program
  - Other environment variables exists too --- covered later
...
TODO: terminology construct vs directive
...
- OpenMP `for`/`do` constructs **distribute work**, that is, assign different threads to different loop iterations (so that the computation work is distributed)
  ...
  - Also other constructs that distribute work exist --- covered later
